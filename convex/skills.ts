...
//
//   1. Find skills no sync touched in ~26h (cheap summary scan).
...
// A stale skill is "healthy" (a live, installable skill we should keep alive) if
// our pipeline currently has a working SKILL.md URL, no content-fetch error, and
// discovery isn't exhausted. Broke skills fail this and are left to delist.
function isReconcileHealthy(s: {
  hasSkillMdUrl: boolean;
  hasContentFetchError: boolean;
  discoveryFailCount: number;
}): boolean {
...
export const reconcileUnseenSkills = internalAction({
  ...
  // Explicit annotation — runQuery/runMutation into internal.skills.* otherwise
  // pulls the whole api type into an inference cycle.
  handler: async (
    ctx,
    args,
  ): Promise<{
    ...
    if (healthyRows.length > batch.length && iteration < MAX_ITER) {
      await ctx.scheduler.runAfter(0, internal.skills.reconcileUnseenSkills, {
        day,
...
export const refreshCuratedSkills = internalAction({
  ...
  handler: async (
    ...
    const page = await ctx.runQuery(internal.skills.listCuratedToRefresh, {
      cursor: args.cursor,
    });
    ...
    if (!page.isDone && iteration < 200) {
      await ctx.scheduler.runAfter(0, internal.skills.refreshCuratedSkills, {
        cursor: page.nextCursor,
        iteration: iteration + 1,
        day,
      });
...
// ---------------------------------------------------------------------------
// One-off cleanup: drop mis-attributed inflated snapshots
// ---------------------------------------------------------------------------
//
...
// ghosts), so this is cheap. Run with { dryRun: true } first.
...
export const cleanupInflatedSnapshots = internalAction({
  ...
  handler: async (
    ...
    let isDone = false;
    while (!isDone) {
      const page = await ctx.runQuery(internal.skills.listLowInstallSummaries, {
        cursor,
      });
      for (const s of page.entries) {
        const res = await ctx.runMutation(internal.skills.pruneInflatedSnapshots, {
          skillDocId: s.skillDocId,
