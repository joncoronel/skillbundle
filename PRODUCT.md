...
**Access model:** A mostly-signed-out public directory. Browsing, searching, viewing catalog pages, and opening shared watchlists need no account. Watching skills doesn't either: signed out, bundles are saved in the browser, with the same change tracking and the free plan's limits, and they move into an account on sign-in. An account adds share links, access from any device, and the paid tier.
...
- Watchlists (no account needed; saved in the browser until sign-in): watch a skill in one click, group watched skills into named lists, unlisted by default with a toggleable share link (account only), copyable install command retained.
