...
| `/skills`, `/skills/[category]`                                                                                             | `○` Static                | The category hub and its 28 category pages. Both read **Typesense**, not Convex, behind `'use cache'` + `cacheLife("days")` (`lib/category-counts.ts`, `lib/category-skills.ts`) — `skillSummaries.tags` is an array and Convex has no array-containment index, and going through the same `searchSkills` the home page's Category filter uses is what keeps the two from disagreeing about a category's contents. The param set is **closed**: `generateStaticParams` returns all 28 keys, so every page prerenders. That does NOT exempt it from the params-into-Suspense rule below — Partial Prefetching builds one App Shell per ROUTE, so this route follows `/[org]` exactly: synchronous page, `params` passed into two boundaries, and only the fixed breadcrumb crumbs plus the "Skill / Installs" column headers reach the shell. `dynamicParams = false` is unavailable under Cache Components; an unknown slug 404s via `categoryKeyFromSlug` returning undefined, which is a soft 404 (200 + `noindex`), same as the other catalog routes. **Typesense is a build-time dependency on a production deployment only** — `lib/typesense-required.ts` throws at module scope there, and elsewhere the loaders return empty so CI (which may have no Typesense secrets) builds green. |
| `/robots.txt`, `/sitemap.xml`                                                                                               | `○` Static                | Crawler-facing metadata routes. robots is a pure function of the route list, and it blocks every non-Overview skill tab (derived from `SKILL_TAB_IDS` in `lib/skill-urls.ts`) because crawling them was the largest ISR-write cost; the sitemap enumerates the whole catalog behind a `'use cache'` loader, so a crawler hit is a CDN read of the XML rather than a ~16-query catalog walk. That loader is **untagged and gated to production** — `cacheLife("days")` refreshes it in the background about once a day, and preview / local builds drain two pages instead of all 16. Both decisions are load-bearing on cost and are argued in full in `app/sitemap.ts`; read that before re-tagging it or removing the gate. Both routes need an absolute origin (`lib/site-url.ts`) because nothing resolves relative URLs in text or XML                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- **Almost everything is params-dependent → `loading.tsx` IS the shell.** On the
  skill detail routes the `[skillId]` layout holds a Suspense-wrapped masthead
  (breadcrumb, `h1` and the tab strip's hrefs are all URL data, so they get a
  shape-matching skeleton whose four always-present tab labels are real text — the strip must NOT
  read `usePathname()` to sidestep that: a Client Component reading URL data
  outside `<Suspense>` is a blocking route under instant validation). The
...
> **OG image caching is separate from the data cache.** The OG routes are dynamic (`ƒ`) because they read `params`, so the `'use cache'` loaders only cache the _Convex data_. The rendered _PNG_ is cached at the CDN via an opt-in `Cache-Control: s-maxage, stale-while-revalidate` header, set by `renderOg(node, { cache: OG_CACHE.DAY })` or `OG_CACHE.YEAR` in `lib/og/templates.tsx`, which replaces the route-level `export const revalidate` that Cache Components disallows. **That header is what keeps images from regenerating on every link**, independent of the data loaders. It is a time header that no `cacheTag` reaches, so its value is how long a card may disagree with the data it was drawn from. The org / repo / source / bundle cards read Convex and take `DAY`. The skill card reads nothing, drawing only from the URL, and takes `YEAR` (Sep 2026, see `skillOgImage` in `lib/og/images.tsx`). The static section cards (`/compare`, `/official`, `/pricing`, root) are `○` and keep Next's build-time static optimization (no header). The brand fonts are read once at module load (`lib/og/fonts.ts`), never inside a render: under Cache Components a render-time `readFile` counts as an async filesystem operation and would flip these otherwise-static routes to `ƒ` (it did, non-deterministically, before the read was hoisted to module scope).
...
Search index in the schema, `withSearchIndex` in queries; search reads go against the slim `skillSummaries` table (~1.3 KB/row) instead of `skills` (~10 KB/row) to keep result sets small on the wire.
...
components/
  ...
  skill-detail-page.tsx     # the Overview tab body + loadStars ('use cache'); shared skill loaders live in lib/skill-cache.ts
