...
| `/robots.txt`, `/sitemap.xml` | `○` Static | Crawler-facing metadata routes. robots is a pure function of the route list; the sitemap enumerates the whole catalog behind a `'use cache'` loader (`cacheLife("days")`, tagged both skill tags), so a crawler hit is a CDN read of ~2 MB of XML rather than a ~10-query catalog walk. Both need an absolute origin (`lib/site-url.ts`) because nothing resolves relative URLs in text or XML |
