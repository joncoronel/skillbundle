...
| `/robots.txt`, `/sitemap.xml`                                                                                               | `○` Static                | Crawler-facing metadata routes. robots is a pure function of the route list; the sitemap enumerates the whole catalog behind a `'use cache'` loader, so a crawler hit is a CDN read of the XML rather than a ~16-query catalog walk. That loader is **untagged and gated to production** — `cacheLife("days")` refreshes it in the background about once a day, and preview / local builds drain two pages instead of all 16. Both decisions are load-bearing on cost and are argued in full in `app/sitemap.ts`; read that before re-tagging it or removing the gate. Both routes need an absolute origin (`lib/site-url.ts`) because nothing resolves relative URLs in text or XML                            |
