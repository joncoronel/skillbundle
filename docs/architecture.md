...
| `/skills`, `/skills/[category]`                                                                                             | `○` Static                | The category hub and its 28 category pages. Both read **Typesense**, not Convex, behind `'use cache'` + `cacheLife("days")` (`lib/category-counts.ts`, `lib/category-skills.ts`) — `skillSummaries.tags` is an array and Convex has no array-containment index, and going through the same `searchSkills` the home page's Category filter uses is what keeps the two from disagreeing about a category's contents. This is the one route family whose param set is **closed**: `generateStaticParams` returns all 28 keys, so every page is fully prerendered and its whole body (h1, blurb, rows) is in the shell — the only catalog-ish route that may `await params` at the top, and `e2e/instant-navigation.spec.ts` asserts it. `dynamicParams = false` is unavailable under Cache Components; an unknown slug 404s via `categoryKeyFromSlug` returning undefined, which is a soft 404 (200 + `noindex`), same as the other catalog routes. **Typesense is a second build-time dependency alongside Convex, but only on a production deployment** — elsewhere the loaders return empty and log, so CI (which has no Typesense secrets) builds green and the two category e2e specs `test.skip()`. See `lib/category-skills.ts`. |
