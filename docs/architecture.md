...
| `/*/opengraph-image`, `/bundle/[id]/og/[v]`, `/api/revalidate` | `ƒ` Dynamic | OG images (data cached via `unstable_cache`); revalidate webhook (secret-gated, called by Convex crons) |
...
> **`'use cache'` vs `unstable_cache`.** On Vercel, `'use cache'` is backed by the Data Cache, so cross-user snapshots persist across requests and instances — there's no need for `unstable_cache` for the home leaderboards, `official`, or the catalog loaders, and dropping it keeps them idiomatic and prerender-friendly (the cached result lands in the static shell). The one place `unstable_cache` remains is the OG-image data loaders (`lib/og/images.tsx`): their `opengraph-image` routes are dynamic (`ƒ`) crawler surfaces, not prerendered shells, so they don't benefit from `'use cache'`'s shell integration. The rendered PNG for the data-backed dynamic (`ƒ`) OG routes (skill / org / repo / source / bundle) is cached at the CDN via an opt-in `Cache-Control: s-maxage=86400, stale-while-revalidate` header — `renderOg(node, { cache: true })` in `lib/og/templates.tsx` — restoring the daily route cache the old `export const revalidate` provided before Cache Components disallowed it. The static section cards (`/explore`, `/compare`, `/official`, `/pricing`, root) are `○` and keep Next's build-time static optimization (no header). The brand fonts are read once at module load (`lib/og/fonts.ts`), never inside a render: under Cache Components a render-time `readFile` counts as an async filesystem operation and would flip these otherwise-static routes to `ƒ` (it did, non-deterministically, before the read was hoisted to module scope).
