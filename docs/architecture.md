...
| `/[org]`, `/[org]/[repo]`, `/[org]/[repo]/[skillId]` (+ its `/history`, `/stats`, `/security`, `/copies` tabs), `/site/...` | `◐` Partial Prerender     | `generateStaticParams` returns one representative param (App Shell prerenders; on the skill segments it lives on the `[skillId]` **layout**, covering the Overview and all three tab pages); unknown params get the shell instantly — from the page's own Suspense fallbacks on the listing routes, from each segment's `loading.tsx` on the skill routes (see "pick one, not both" below) — then upgrade. Data via `'use cache'` loaders split across two tags — `cacheTag('skill-sync')` for install/version data, `cacheTag('skill-content')` for the skill row (`lib/skill-cache.ts`). **These pages must not `await params` above their Suspense boundaries** — see "Params and the shared App Shell" below |
